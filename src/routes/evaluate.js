...
/**
 * POST /ai/evaluate/job
 * Evaluate CV against a job description
 */
router.post("/job", validate(schemas.job), async (req, res) => {
  try {
    const { userId, sessionId, jobDescription } = req.body;
    ...
    const result = await ai.evaluateJobMatch(sessionId, jobDescription);
    ...
    res.status(500).json({
      success: false,
      error: error.message || "Failed to evaluate job match",
    });
  }
});
